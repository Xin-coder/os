use crate::lib::consts::MAX_PHYSICAL_PAGES;
use spin::Mutex;

pub struct SegmentTreeAllocator {
    a: [u8; MAX_PHYSICAL_PAGES << 1], // 页表占用标志？
    m: usize,
    n: usize,
    offset: usize
}
...
impl SegmentTreeAllocator {
    ...
    pub fn init(&mut self, l: usize, r: usize) {
        ...
        self.n = r - l;
        self.m = 1;
        while self.m < self.n + 2 { // 为什么是n+2?感觉有点浪费？极限情况m<<1是否会大于MAX_PHYSICAL_PAGES << 1
            self.m = self.m << 1;
        }
    ...
    pub fn alloc(&mut self) -> usize {
        ...
        let mut p = 1;
        while p < self.m {
            if self.a[p << 1] == 0 { p = p << 1; } else { p = (p << 1) | 1; } //通过节点可以知道子节点是否分配
        }
